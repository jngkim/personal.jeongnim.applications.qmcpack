...
echo "mpirun ${MPI_BIND_OPTIONS} -np ${ppn} ${GPU_AFFINITY}  ${app}  ${input} --enable-timers=fine"
mpirun ${MPI_BIND_OPTIONS} -np ${ppn} ${GPU_AFFINITY}  ${app}  ${input} --enable-timers=fine  2>&1 | tee -a qmcpack.out
